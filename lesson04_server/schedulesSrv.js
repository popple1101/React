...
/*
  curl -X GET http://localhost:5001/api/schedules/2025-07-09
...
app.delete("/api/schedules/", async (req, res) => {
  try {
    ...
    if (time) { // time 값이 있어요. date 특정 날짜의 time 을 삭제
      // 시간 형식 검증 (예: 13:00)
...
curl -X PATCH http://localhost:5001/api/schedules ^
  -H "Content-Type: application/json" ^
  -d "{  \"date\": \"2025-07-09\", \"time\": \"15:00\", \"checked\": false }"
...
app.patch('/api/schedules', async(req,resp) => {
  ...
  try {
    ...
    const timeRegex = /^\d{2}:\d{2}$/;
      ...
      // db 요청 : checked 수정(updateOne 메소드)
      const collection = db.collection(COLLECTION_NAME)
      ...
      // 응답 상태코드 지정(서버 오류 500). 오류 메시지 보내기 -> 리액트에서 받는 응답
      response.status(500).json({error : "서버오류가 발생했습니다."})
  } 
})